...
export const IngestionFlow = ai.defineFlow({
    ...
    inputSchema: z.object({
        ...
        lang: z.string().describe("Language Code (e.g. 'en' for English, 'he-IL' for Hebrew)")
    })
    },
    async (input: { url: string, lang: string}) => {

        const bm25engine = bm25();
        // Configure field weights for BM25 search
        ...
        // // Add a sample document to initialize the fields
        // bm25engine.addDoc({
        //   text: '',
        //   title: '',
        //   url: ''
        ...
        const processUrl = async (url: SitemapUrl) => {
            try {
                ...
                for (const block of contentBlocks) {
                    ...
                    // Add the chunk to BM25
                    const docId = bm25engine.getTotalDocs() + 1;
                    bm25engine.addDoc({ text, title, url: url.loc }, docId);
                    logger.debug(`Chunk id added to BM25: ${docId}`);
        ...
        const indexData = bm25engine.exportJSON();
        const indexPath = process.env.BM25_STORE_NAME || "./bm25_index.json";
        await fs.writeFile(indexPath, JSON.stringify(indexData));
        logger.info(`BM25 index saved to ${indexPath}`);
